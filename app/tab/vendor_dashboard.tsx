...
export default function VendorDashboard() {
    ...
    async (showLoading = false) => {
      ...
      try {
        ...
        if (
          ...
          ordersData.length > previousOrdersCountRef.current
        ) {
          Toast.show({
            type: "success",
            text1: t("newOrder"),
            text2: t("newOrderReceived"),
            position: "top",
          });
        }
  ...
  const toggleShopStatus = async () => {
    ...
    try {
      ...
      const success = await updateVendorProfile(user?.id!, {
        isOpen: newStatus,
      });
      if (success) {
        setDbUser((prev: any) => ({
          ...prev,
          vendorProfile: { ...prev.vendorProfile, isOpen: newStatus },
        }));
      }
    } finally {
  ...
  const handleSaveProduct = async () => {
    if (!productForm.name || !productForm.description || !productForm.price) {
      Alert.alert(t("incomplete"), t("pleaseComplete"));
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        Toast.show({
          type: "success",
          text1: editingProduct ? "Produit modifié !" : "Produit ajouté !",
          position: "top",
        });
      } else {
      ...
      {products.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubText}>
            Ajoutez vos spécialités ci-dessus
          </Text>
        </View>
      ) : (
        ...
        <View style={styles.settingsCard}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText}>
              {dbUser.vendorProfile.description || "Aucune description"}
            </Text>
          </View>
          {dbUser.vendorProfile.specialty?.length > 0 && (
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>
                {dbUser.vendorProfile.specialty.join(", ")}
              </Text>
            </View>
          )}
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.alert(
                "Info",
                "Retournez sur l'inscription vendeur pour modifier vos infos.",
              )
            }
          >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabBar}>
        {(
          [
            { key: "orders", label: t("ordersTab"), icon: "receipt-outline" },
            {
              key: "products",
              label: t("productsTab"),
          ...
          >
            <Ionicons
              name={tab.icon}
              size={20}
              color={activeTab === tab.key ? "#FFC700" : "#999"}
            />
            <Text
              style={[
                styles.tabLabel,
